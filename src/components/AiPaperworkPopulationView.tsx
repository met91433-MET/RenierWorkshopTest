    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const altResponse = await fetch('/api/parse-delivery-image', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              images: [imageDataUrl],
      ...
      if (data.matchedCustomerId && customers.some(c => c.id === data.matchedCustomerId)) {
        ...
        const query = data.customerName.toLowerCase();
        const found = customers.find(c => 
          c.name.toLowerCase().includes(query) ||
          query.includes(c.name.toLowerCase())
        );
        if (found) setSelectedCustomerId(found.id);
        else if (customers[0]) setSelectedCustomerId(customers[0].id);
